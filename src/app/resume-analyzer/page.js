...
export default function ResumeAnalyzerPage() {
  ...
  useEffect(() => {
    const loadProfileResume = async () => {
      try {
        console.log('Loading profile resume...');
        ...
        console.log('Profile data loaded:', {
          hasParsedResumeText: !!data.parsedResumeText,
          textLength: data.parsedResumeText?.length
        });
        
        if (data.parsedResumeText) {
          console.log('Setting resume text from profile');
          setResumeText(data.parsedResumeText);
        } else {
          console.log('No parsed resume text found in profile');
        }
      } catch (error) {
        console.error('Error loading profile resume:', {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      // First, analyze the resume with Gemini
            ...
            <div className="border-t border-gray-200 p-6 bg-gray-50">
              ...
              <div className="space-y-8">
                <div>
                  ...
                  {analysis.roleAnalysis?.length > 0 ? (
                    <div className="space-y-4">
                      {analysis.roleAnalysis.map((role, index) => (
                        <div key={index} className="bg-white p-4 rounded-lg shadow">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-medium text-gray-900">{role.roleTitle || 'Software Developer'}</h4>
                              <p className="text-sm text-gray-500">
                                Match Score: {role.matchPercentage || 0}%
                              </p>
                            </div>
                          ...
                          <div className="mt-2">
                            {renderSkillMeter(role.matchPercentage || 0)}
                          </div>
                          <div className="mt-3">
                            <p className="text-sm text-gray-700">
                              {role.justification || 'Analysis in progress...'}
                            </p>
                          </div>
                            ...
                            <div className="mt-4">
                              ...
                              <ul className="space-y-2">
                                {role.skillGaps.map((gap, gapIndex) => (
                                  <li key={gapIndex} className="text-sm text-gray-600">
                                        ...
                                        {gap.suggestions.map((suggestion, sIndex) => (
                                          <li key={sIndex} className="flex items-start">
                                            ...
                                            <span>
                                              ...
                                              {suggestion.platform && (
                                                <span className="text-gray-500 text-xs ml-2">({suggestion.platform})</span>
                                              )}
                                            </span>
                                          </li>
                                        ))}
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-lg">
                      <p className="text-gray-500">No role matches found. Try analyzing your resume.</p>
                    </div>
                  )}
                </div>
